int readtxt(const char *filename, PKMN *pokemonList) {
    ...
    while (count < MAX_POKEMON){
	    ...
	    if (result == EOF) {
	        ...
	        printf("Error al leer la l�nea %d. Resultados: %d\n", count + 1, result);
	        break; // Error al leer, puede que la l�nea est� mal formada
	    }
	
	    // Manejar el caso donde type2 puede estar vac�o
	    if (strcmp(pokemonList[count].type2, "-") == 0) {
	        strcpy(pokemonList[count].type2, "----"); // Asignar cadena vac�a si es un guion
	    }
	    count++;
	}
    return count; // Retorna la cantidad de Pok�mon le�dos
}
...
int addPokemon(const char *filename, PKMN *pokemonList, int *totalPokemons) {
    ...
    // Si el segundo tipo es "-", lo dejamos vac�o
    if (strcmp(newPokemon.type2, "-") == 0) {
        strcpy(newPokemon.type2, "");
    }

    // A�adir el Pok�mon al arreglo
    pokemonList[*totalPokemons] = newPokemon;
    (*totalPokemons)++;

    // Guardar el nuevo Pok�mon en el archivo
    FILE *file = fopen(filename, "a");
    if (file == NULL) {
        perror("Error al abrir el archivo para a�adir el Pok�mon");
        return -1;
    }
    ...
    printf("Pok�mon a�adido con �xito.\n");
    return 0;
}
